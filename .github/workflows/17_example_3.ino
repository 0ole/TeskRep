#include <Servo.h>

// Arduino pin assignment
#define PIN_IR    A0        // IR sensor at Pin A0
#define PIN_LED   9
#define PIN_SERVO 10

// Servo pulse width range (μs)
#define _DUTY_MIN 500    // 0 degree
#define _DUTY_NEU 1500   // 90 degree
#define _DUTY_MAX 2500   // 180 degree

// Distance range (mm)
#define _DIST_MIN  100.0   // 10 cm
#define _DIST_MAX  250.0   // 25 cm

// Filter & Timing parameters
#define EMA_ALPHA  0.2      // EMA filter alpha
#define LOOP_INTERVAL 20    // loop interval (msec)

Servo myservo;
unsigned long last_loop_time = 0;

float dist_ema = _DIST_MIN;

void setup()
{
  pinMode(PIN_LED, OUTPUT);
  
  myservo.attach(PIN_SERVO); 
  myservo.writeMicroseconds(_DUTY_NEU);
  
  Serial.begin(1000000);    // High baud rate for Serial Plotter
}

void loop()
{
  unsigned long time_curr = millis();
  int duty;
  float a_value, dist_raw, dist_filtered;

  // wait until next event time
  if (time_curr < (last_loop_time + LOOP_INTERVAL))
    return;
  last_loop_time = time_curr;

  // Read IR sensor
  a_value = analogRead(PIN_IR);
  dist_raw = ((6762.0 / (a_value - 9.0)) - 4.0) * 10.0 - 60.0;  // mm 단위

  // ---- Range Filter ----
  if (dist_raw < _DIST_MIN || dist_raw > _DIST_MAX) {
    digitalWrite(PIN_LED, LOW);   // 범위 밖 → LED OFF
    dist_filtered = dist_ema;     // 이전 값 유지 (EMA 안정성)
  } else {
    digitalWrite(PIN_LED, HIGH);  // 범위 안 → LED ON
    dist_filtered = dist_raw;
  }

  // ---- EMA Filter ----     
  dist_ema = EMA_ALPHA * dist_filtered + (1 - EMA_ALPHA) * dist_ema;

  // ---- Linear Mapping (no map() function) ----
  // duty = _DUTY_MIN + (dist_ema - _DIST_MIN) * (_DUTY_MAX - _DUTY_MIN) / (_DIST_MAX - _DIST_MIN);
  duty = _DUTY_MIN + ((dist_ema - _DIST_MIN) * (_DUTY_MAX - _DUTY_MIN)) / (_DIST_MAX - _DIST_MIN);

  // ---- Servo Control ----
  myservo.writeMicroseconds(constrain(duty, _DUTY_MIN, _DUTY_MAX));

  // ---- Serial Plotter Output ----
  Serial.print("_DUTY_MIN:");  Serial.print(_DUTY_MIN);
  Serial.print(",_DIST_MIN:"); Serial.print(_DIST_MIN);
  Serial.print(",IR:");        Serial.print(a_value);
  Serial.print(",dist_raw:");  Serial.print(dist_raw);
  Serial.print(",ema:");       Serial.print(dist_ema);
  Serial.print(",servo:");     Serial.print(duty);
  Serial.print(",_DIST_MAX:"); Serial.print(_DIST_MAX);
  Serial.print(",_DUTY_MAX:"); Serial.print(_DUTY_MAX);
  Serial.println("");
}
